...
### 5.3 adapter.in.scheduler

- `@Scheduled` 진입점을 둔다.
- 크론 표현식은 설정 파일에서 주입받는다.
- 여기서는 시작 요청만 하고 실제 로직은 유스케이스로 위임한다.
...
- `@EnableScheduling` 사용
- 크론 표현식은 `application-*.yml` 로 분리
- 스케줄러 클래스는 배치 트리거 역할만 수행
- 실제 작업 메서드와 스케줄 어노테이션 메서드를 분리

단일 인스턴스 운영이 아니면 중복 실행 방지 전략을 추가 검토한다.
확장 후보:

- ShedLock
- DB Lock 테이블
- 운영 배치 스케줄러와 애플리케이션 역할 분리
